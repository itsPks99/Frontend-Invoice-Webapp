...
export default function LandingPage() {
  ...
  return (
    <div className="App">
      {/* Navbar */}
      <nav
        className={`fixed top-0 w-full z-20 transition-all duration-300 ${scrolled ? "bg-white shadow-md h-16" : "bg-transparent h-20"
          }`}
      >
        <div
          className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-all duration-300 ${scrolled ? "h-16" : "h-20"
            }`}
        >
          {/* Logo Section */}
          <div className="">
      ...
      {/* Hero Section */}
      <section
        className={`bg-gradient-to-r from-blue-500 via-blue-400 to-blue-300 h-screen flex items-center overflow-hidden transition-all duration-300 ${scrolled ? 'pt-16' : 'pt-20'
          }`}
        style={{
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-6">
            {TABS.map((tab, index) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border-2 font-semibold transition duration-200 transform ${index === activeTab
